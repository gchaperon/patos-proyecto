...
object PlusOneMinusOne {
  def main(args: Array[String]) {
    val filePath = "/home/gabriel/2019-1/patos/proyecto/data/root_all_wrepeat_tsdate_fixed.tsv"
    val spark = new SparkContext(new SparkConf().setAppName("Count +1/-1"))

    val rootComments = spark.textFile(filePath)

    val countAll = rootComments.filter(line => line.split("\t")(1).contains("+1/-1")).count()
